...
class Experts_channel(nn.Module):
    def __init__(self,
                 dim,
                 num_experts=8,
                 hidden_dim=None,
        ...
        self.w1 = nn.Parameter(init_(torch.zeros(*num_experts, dim, hidden_dim)))
        self.w2 = nn.Parameter(init_(torch.zeros(*num_experts, hidden_dim, dim)))
        self.act = activation()
        self.sigmoid = nn.Sigmoid()
        self.softmax = nn.Softmax(dim=-1)

        self.mlp1 = nn.Parameter(init_(torch.zeros(*num_experts, dim, hidden_dim)))
        self.mlp2 = nn.Parameter(init_(torch.zeros(*num_experts, hidden_dim, dim)))

    def forward(self, x):
        gap = x.mean(dim=1, keepdim=True)  # (B, 1, D)
        attention_ = torch.einsum('...nd,...dh->...nh', gap, self.mlp1)
        attention_ = self.act(attention_)
        attention = torch.einsum('...nd,...dh->...nh', attention_, self.mlp2)
        attention = self.softmax(attention)  # (B, 1, D)
        x = x * attention + x  # (B, N, D)
        hidden = torch.einsum('...nd,...dh->...nh', x, self.w1)
        hidden = self.act(hidden)
        out = torch.einsum('...nh,...hd->...nd', hidden, self.w2)
        return out
...
# gating network
def sparse_softmax_top2(logits: torch.Tensor) -> torch.Tensor:
    """
    Computes a sparse softmax where the probability mass is concentrated
    on the top-2 logits.

    Args:
        logits: The raw scores from the gating network.
                Shape: [..., num_experts].

    Returns:
        A tensor with the same shape as logits, where probabilities
        are non-zero only for the top-2 elements in the last dimension.
    """
    top2_values, top2_indices = torch.topk(logits, k=2, dim=-1)
    sparse_logits = torch.full_like(logits, float('-inf'))
    sparse_logits.scatter_(-1, top2_indices, top2_values)
...
class Top2Gating(nn.Module):
    def __init__(
        ...
        self.w_noise = nn.Parameter(torch.randn(*outer_expert_dims, dim, num_gates))
    ...
    def forward(self, x, importance=None):
        *_, b, group_size, dim = x.shape
        ...
        if self.training:
            ...
            noise_strength = F.softplus(torch.einsum('...bnd,...de->...bne', x, self.w_noise))
            noise = torch.randn_like(noise_strength) * noise_strength
        ...
        density_2 = mask_2.mean(dim=-2)
        # Something continuous that is correlated with what we want to equalize.
        density_1_proxy = density_1_proxy.mean(dim=-2)
        loss = (density_1_proxy * (density_1 + density_2) / 2).mean() * float(num_gates ** 2)
...
# mixture of Group Attention Experts
class MoE_channel(nn.Module):
    def __init__(self,
                 dim,
                 num_experts,
                 hidden_dim=None,
        ...
        gating_kwargs = {'second_policy_train': second_policy_train, 'second_policy_eval': second_policy_eval,
                         ...
                         'capacity_factor_eval': capacity_factor_eval}
        self.gate = Top2Gating(dim, num_gates=num_experts, **gating_kwargs)
        self.experts = default(experts, lambda: Experts_channel(dim, num_experts=num_experts, hidden_dim=hidden_dim,
                                                                activation=activation))
        self.loss_coef = loss_coef
    ...
    def forward(self, inputs, **kwargs):
        b, n, d, e = *inputs.shape, self.num_experts
        dispatch_tensor, combine_tensor, loss = self.gate(inputs)
        expert_inputs = torch.einsum('bnd,bnec->ebcd', inputs, dispatch_tensor)  # [e,b,40,768]
        # Now feed the expert inputs through the experts.
        orig_shape = expert_inputs.shape
        expert_inputs = expert_inputs.reshape(e, -1, d)
        expert_outputs = self.experts(expert_inputs)
