...
from networks.segment_anything_main.segment_anything.modeling.common import LayerNorm2d, MLPBlock
from networks.segment_anything_main.segment_anything.MM_SAM.moe_block import MoEBlock
...
class ImageEncoderViT(nn.Module):
    def __init__(
            ...
            scale_factor=16,  # 缩放因子,768 --> 768 // scale_factor
    ) -> None:
        """
        Args:
        ...
        self.depth = depth
        self.fusion_Adapter = nn.Conv2d(in_channels=6, out_channels=3, kernel_size=3, padding=1)
    ...
    def forward(self, x: torch.Tensor) -> Tuple[Any, Any, Any, Any, Any, Union[int, Any]]:
        x = self.fusion_Adapter(x.contiguous())
        ...
        for i in range(self.depth):
            blk = self.blocks[i]
            x, aux_loss_i = blk(x, i)
            aux_loss = aux_loss + aux_loss_i
            if (i + 1) % 3 == 0:
                res.append(x)

        x = self.neck(x.permute(0, 3, 1, 2))
        return x, res[3], res[2], res[1], res[0], aux_loss  # neck output, 12, 9, 6, 3, aux_loss
